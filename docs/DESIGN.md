...
```bash
swift run paint-demo --banner docs/images/quick-sketch-window.png docs/images/banner.png
```

`ReadmeBanner` composes it out of PaintKit — the gradient is `Bitmap.fill` per
row, the wordmark is `TextRenderer`, the window is `ImageTransform.scaled` and
composited so it bleeds off the bottom edge. So the page header is reproducible
from a clean checkout, it moves when the icon or the interface moves, and it is
itself a demonstration that the engine composes images. A hand-edited PNG sitting
in `docs/images` would be none of those.
