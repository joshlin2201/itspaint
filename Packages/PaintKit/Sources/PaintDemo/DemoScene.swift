...
enum DemoScene: String, CaseIterable, Sendable {
    ...
    func render() -> DemoCanvas {
        let canvas = DemoCanvas(fill: self == .transparency ? .clear : .white)
        switch self {
        case .clipboard:
            drawClipboard(on: canvas)
        case .quickSketch:
            drawQuickSketch(on: canvas)
        case .transparency:
            drawTransparency(on: canvas)
        }
        return canvas
    }

    @MainActor
    private func drawClipboard(on canvas: DemoCanvas) {
    ...
    @MainActor
    private func drawQuickSketch(on canvas: DemoCanvas) {
        let engine = canvas.engine
        let navy = canvas.colour("17324D")
        let blue = canvas.colour("2F80ED")
        let yellow = canvas.colour("FFD166")
        ...
        canvas.text("From thought to sketch.", in: PixelRect(x: 64, y: 58, width: 700, height: 60), size: 42, colour: navy, bold: true)
        canvas.text("A few marks are enough to make an idea visible.", in: PixelRect(x: 66, y: 118, width: 680, height: 34), size: 20, colour: slate, bold: false)

        engine.settings.tool = .highlighter
        engine.settings.brushSize = 26
        engine.settings.highlighterOpacity = 0.45
        engine.colours.foreground = yellow
        canvas.drag(from: PixelPoint(x: 70, y: 197), to: PixelPoint(x: 572, y: 197))
        ...
        engine.colours.foreground = blue
        canvas.drag(
            from: PixelPoint(x: 100, y: 430),
            to: PixelPoint(x: 870, y: 390),
            via: [PixelPoint(x: 260, y: 300), PixelPoint(x: 430, y: 470), PixelPoint(x: 620, y: 280)]
        )
        ...
        engine.settings.brushSize = 4
        engine.colours.foreground = navy
        canvas.shape(.arrow, from: PixelPoint(x: 160, y: 510), to: PixelPoint(x: 770, y: 285))
        canvas.text("Start simple.", in: PixelRect(x: 110, y: 540, width: 340, height: 36), size: 26, colour: navy, bold: true)
    }

    @MainActor
    private func drawTransparency(on canvas: DemoCanvas) {
        let engine = canvas.engine
        let navy = canvas.colour("17324D")
        let blue = canvas.colour("2F80ED")
        let coral = canvas.colour("EF476F")
        let yellow = canvas.colour("FFD166")

        engine.settings.tool = .shape
        engine.settings.shapeStyle = .filled
        engine.colours.background = blue
        canvas.shape(.ellipse, from: PixelPoint(x: 170, y: 150), to: PixelPoint(x: 610, y: 530))
        engine.colours.background = coral
        canvas.shape(.ellipse, from: PixelPoint(x: 390, y: 150), to: PixelPoint(x: 830, y: 530))

        engine.settings.tool = .highlighter
        engine.settings.brushSize = 42
        engine.settings.highlighterOpacity = 0.48
        engine.colours.foreground = yellow
        canvas.drag(from: PixelPoint(x: 260, y: 340), to: PixelPoint(x: 750, y: 340))

        engine.settings.tool = .shape
        engine.settings.shapeStyle = .outline
        engine.settings.brushSize = 5
        engine.colours.foreground = navy
        canvas.shape(.roundedRectangle, from: PixelPoint(x: 100, y: 90), to: PixelPoint(x: 900, y: 570))
    }
}
